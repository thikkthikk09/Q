  ...
  if (window.Khqr) {
    ...
    document.getElementById('claimTopupBtn')?.addEventListener('click', async () => {
      ...
      if (manual && /^[a-f0-9]{32}$/.test(manual)) {
        Khqr.addPaymentHistory?.(manual, selectedTopup)
        const ok = await Khqr.checkMd5AndCredit?.(manual, selectedTopup)
        if (ok) {
          if (btn) btn.disabled = false
          return
        }
      ...
      if (!ok) showToast('Bakong: payment not found yet — use same browser & QR you paid')
      if (btn) btn.disabled = false
    })
  }
