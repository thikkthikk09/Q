...
;(function (global) {
  ...
  function addPaymentHistory(md5, usd) {
    ...
    list.push({ md5: hash, usd: Number(usd) || 0.01, at: Date.now() })
  ...
  function savePendingTopup() {
    ...
    const record = { md5: currentMd5, usd: currentUsd, at: Date.now() }
    localStorage.setItem(PENDING_KEY, JSON.stringify(record))
    addPaymentHistory(currentMd5, currentUsd)
  }
  ...
  function creditBalanceDirect(amount) {
    ...
    const next = (Number.isFinite(bal) && bal >= 0 ? bal : 0) + Number(amount)
    localStorage.setItem(key, String(next))
    if (typeof global.onDynaTopupSuccess === 'function') {
      global.onDynaTopupSuccess(amount, {})
    } else if (global.Khqr?.onPaymentSuccess) {
      global.Khqr.onPaymentSuccess(amount, {})
    }
    global.showKhqrToast?.(`+${formatUsd(amount)} added · Balance $${next.toFixed(2)}`)
  ...
  async function checkMd5AndCredit(md5, usd) {
    ...
    currentMd5 = hash
    currentUsd = Number(usd) || 0.01
    activeProxyOrigin = 'direct'

    applyConfigCredentials()
    if (!canUseDirectBakong()) await renewJwtDirect()

    try {
      let json = await checkTransactionByMd5(hash)
      let st = parsePaymentStatus(json)
      saveLastCheck(hash, st, json.responseMessage || '')

      if (st === 'no_token' || st === 'unauthorized') {
        await renewJwtDirect()
        json = await checkTransactionByMd5(hash)
        st = parsePaymentStatus(json)
        saveLastCheck(hash, st, json.responseMessage || '')
      }

      if (st === 'paid') {
        return applyTopupCredit(usd || currentUsd, hash, json.data)
      }
    } catch (err) {
  ...
  function updatePendingBanner(show) {
    ...
    if (md5El) md5El.textContent = md5 ? `${md5.slice(0, 8)}…${md5.slice(-4)}` : '—'

    const statusEl = document.getElementById('pendingCheckStatus')
    if (statusEl) {
      try {
        const last = JSON.parse(localStorage.getItem(LAST_CHECK_KEY) || '{}')
        if (last.status === 'paid') statusEl.textContent = 'Bakong: paid — adding balance'
        else if (last.detail) statusEl.textContent = `Last check: ${last.detail}`
  ...
  async function discoverProxy() {
    ...
    if (isStaticHosting()) {
      if (!canUseDirectBakong() && getBakongEmail() && getRegisterCode()) {
        try {
          await renewJwtDirect()
        } catch (e) {
  ...
  async function checkTransactionByMd5(md5Hash) {
    ...
    async function callDirect() {
      let bearer = token
      if (!bearer.startsWith('eyJ') && getRegisterCode()) {
        bearer = await renewJwtDirect()
      }
      if (!bearer.startsWith('eyJ')) throw new Error('NO_TOKEN')
      const res = await fetch(`${PAYMENT_CHECK.apiBase}/v1/check_transaction_by_md5`, {
        method: 'POST',
        mode: 'cors',
        headers: {
      ...
      if (json.errorCode === 6) {
        ...
        if (renewed) {
          const retry = await fetch(`${PAYMENT_CHECK.apiBase}/v1/check_transaction_by_md5`, {
            method: 'POST',
            mode: 'cors',
            headers: {
              'Content-Type': 'application/json',
          ...
          const paid = parsePaymentStatus(retryJson) === 'paid'
          return { ...retryJson, _dyna: { paid, md5, hasJwt: true, direct: true } }
        }
    ...
    if (proxy && !proxy.includes('127.0.0.1:8787')) {
      try {
  ...
  async function ensurePaymentWatcher() {
    applyConfigCredentials()
    await discoverProxy()
    if (canUseDirectBakong()) return true
    if (isStaticHosting()) {
      if (getRegisterCode()) await renewJwtDirect()
      return canUseDirectBakong()
    }
  ...
  async function runPaymentCheck() {
    ...
    try {
      ...
      if (!canUseDirectBakong() && !configApiBase()) {
        setPaymentStatus('pending', 'Payment API not ready — refresh page')
        return
      }
      ...
      if (status === 'paid') {
        applyTopupCredit(currentUsd, currentMd5, json.data)
